...
%macro for method
\newcommand{\methods}{LT-learners\xspace}
...
\begin{abstract}
Estimation of heterogeneous \textit{long-term} treatment effects (HLTEs) is widely used for personalized decision-making in marketing, economics, and medicine, where short-term randomized experiments are often combined with long-term observational data. However, HLTE estimation is challenging due to limited overlap in treatment or in observing long-term outcomes for certain subpopulations, which can lead to unstable HLTE estimates with large finite-sample variance. To address this challenge, we introduce the \textbf{LT-learners}, a set of novel orthogonal learners for HLTE estimation. Our LT-learners are designed for the canonical HLTE setting that combines a short-term randomized dataset $\mathcal{D}_1$ with a long-term historical dataset $\mathcal{D}_2$. The key idea of our LT-learners is to \textit{retarget} the learning objective by introducing custom overlap weights that downweight samples with low overlap in treatment or in long-term observation. We show that the retargeted loss is equivalent to the weighted oracle loss and satisfies Neyman-orthogonality, which means our learners are robust to errors in the nuisance estimation. We further provide the error bound for the \methods and give the conditions under which quasi-oracle rate can be achieved. Finally, our LT-learners are model-agnostic and can thus be instantiated with arbitrary machine learning models. We conduct empirical evaluations on synthetic and semi-synthetic benchmarks to confirm the theoretical properties of our \methods, especially the robustness in low-overlap settings. To the best of our knowledge, ours are the first orthogonal learners for HLTE estimation that are robust to low overlap that is common in long-term outcomes.  
\end{abstract}
...
Several methods have been developed for estimating long-term treatment effects~\citep[e.g.,][]{Athey.2016,Chen.2023,Athey.2025}, but they primarily focus on the \textit{average} long-term treatment effect (ALTE) rather than the HLTE. Surprisingly, learners designed for the canonical HLTE are missing in the literature. Nevertheless, some of the approaches from the literature can be adapted by leveraging the derived efficient influence functions (we later show this in \Cref{sec:standard_learners}. In particular, building on the methodology of \citet{Chen.2025}\footnote{Although \citet{Chen.2025} consider a slightly different data setting, the derivation of the long-term RA-, IPW-, and DR-learners for HLTE follows the same principles.}, such extensions give rise to a class of two-stage meta-learners\footnote{Meta-learners are model-agnostic algorithms that decompose heterogeneous treatment effect estimation into a collection of sub-regression problems \cite{Kunzel.2019, Curth.2021}.} tailored to the HLTE setting. These learners differ in how they adjust for confounding, including (1)~regression adjustment (\textbf{long-term RA-learner}), (2)~inverse propensity weighting (\textbf{long-term IPW-learner}), and (3) doubly robust estimation (\textbf{long-term DR-learner}). 
...
In this paper, we address these challenges by proposing the \textbf{\methods} (long-term-learner), a novel class of orthogonal meta-learners for estimating HLTEs from a short-term randomized dataset $\mathcal{D}_1$ and a long-term observational dataset $\mathcal{D}_2$. The key idea behind our \methods is to explicitly account for both (i)~limited treatment assignment overlap and (ii) limited long-term outcome overlap. To this end, our \methods \textit{retarget} the learning objective through carefully designed overlap weights that downweight samples with weak treatment overlap or low probability of observing long-term outcomes, thereby stabilizing the second-stage learning problem. We show that the retargeted loss is equivalent to the weighted oracle loss. Then, we instantiate three variants: (1)~\textbf{TO-LT-learner}, (2)~\textbf{LO-LT-learner}, and (3)~\textbf{DO-LT-learner} with different weightings tailored for different overlap regimes. We further provide an error bound for the \methods and give the conditions under which a quasi-oracle convergence rate can be achieved. Importantly, our \methods are fully model-agnostic: they can be instantiated with arbitrary machine learning models (e.g., neural networks) for both nuisance estimation and outcome learning, making them broadly applicable in high-dimensional HLTE settings from practice.
...
Our \textbf{contributions} are:\footnote{Code is available at \href{https://anonymous.4open.science/r/LT-learner-for-Long-term-HTE-5041}{https://anonymous.4open.science/r/LT-learner-for-Long-term-HTE-5041}. Upon acceptance, we move the code to a public GitHub repository.} (1)~We propose the \textit{LT-learners}, a novel class of orthogonal meta-learners for estimating HLTEs, which are designed to be robust to different forms of low overlap. (2)~We provide theoretical guarantees on the convergence rate of our \methods. (3)~We conduct extensive experiments on both synthetic and semi-synthetic datasets to demonstrate the robustness of our \methods in low-overlap regimes.
...
Interestingly, there is no estimator for the canonical HLTE setting. Nevertheless, such estimators can be readily extended from the literature (we do so in \Cref{sec:standard_learners}). In particular, building on the results from \citet{Chen.2025}, we later formalize several learners that are tailored to the HLTE setting but employ different adjustment strategies, namely, (1)~regression adjustment (\textbf{long-term RA-learner}), (2)~inverse propensity weighting (\textbf{long-term IPW-learner}), and (3) doubly robust estimation (\textbf{long-term DR-learner}). \textit{However}, these meta-learners have limitations: The long-term RA-learner is subject to \textit{plug-in bias}~\cite{Kennedy.2022-Sempiparametric}. The long-term IPW-learner suffers from \textit{high finite-sample variance} under low treatment overlap due to divisions by near-zero values. While the DR-learner mitigates first-order bias from nuisance estimation, it is \textit{sensitive to (i)~low treatment overlap and (ii)~low long-term outcome overlap in finite samples} (i.e., when outcomes are only sparsely available for certain subgroups). 
...
\textbf{Research gap:} Existing methods for HLTE estimation fail to address the challenge due to (i)~limited treatment overlap and (ii)~limited long-term observation overlap issue in finite samples (see Table~\ref{tab:related-works}). To the best of our knowledge, we are the first to provide a general class of orthogonal learners that is robust in low-overlap regimens. 
...
\textbf{Setup:} We follow the canonical long-term outcome setting \citet{Athey.2025, Chen.2023} but which we adapt from ALTEs to HLTEs. We thus consider two datasets: a short-term experimental dataset $\mathcal{D}_1$ and a long-term historical observational dataset $\mathcal{D}_2$, with $N_1$ and $N_2$ units respectively.  $\mathcal{D}_1$ and $\mathcal{D}_2$ differ in terms of which variables are observed (see Figure~\ref{fig:concept}). 
...
The covariates $X$ and short-term surrogates $S$ are observed in both $\mathcal{D}_1$ and $\mathcal{D}_2$. The long-term outcome $Y$ is observed only in $\mathcal{D}_2$. Then, the treatment $A$ is only measured in the experimental dataset $\mathcal{D}_1$ but \underline{not} in $\mathcal{D}_2$ (e.g., because this is a new treatment that was not applied previously). Formally, we can combine $\mathcal{D}_1$ and $\mathcal{D}_2$ to build a unified dataset with $N=N_1+N_2$ units:
$%\begin{align*}
...
\label{assumption-positivity} \textit{The treatment overlap is nondegenerate: $0< P(A=1 \mid X=x)<1, \; \forall x\in \mathcal{X}$.}
\end{assumption}
...
A formal proof for Eq.~(\ref{eq:identification-tau0}) is provided in Appendix~\ref{app:identification}. %The identification of the average treatment effects is shown in~\cite{Athey.2025}. Here we adopt the same setting and then extend the results to the heterogeneous effects.
...
\section{T-, RA-, IPW-, and DR-learner for HLTE estimation}
...
\centering
\tiny
\caption{\textbf{Overview of meta-learners for HLTE estimation.} Our \methods are the only approach that is both Neyman-orthogonal and robust to different types of low-overlap scenarios.}
...
& \makecell{\textbf{Robust to low} \\ \textbf{LT outcome overlap}} \\
\midrule
Long-term T-learner
& \xmark & \xmark & \xmark \\
Long-term RA-learner
& \xmark & \xmark & \xmark \\
Long-term IPW-learner
& \xmark & \xmark & \xmark \\
Long-term DR-learner
& \cmark & \xmark & \xmark \\
\midrule
\textbf{\methods (\emph{ours})}
& \cmark & \cmark & \cmark \\
\bottomrule
\end{tabular}
}
...
In this section, we briefly introduce the RA-, IPW-, and DR-learner for the HLTE setting (see Table~\ref{tab:related-works}), which can be obtained by extending standard HTE estimators to the long-term setting using the identification result in Eq.~(\ref{eq:identification-tau0}). To do this, we simply replace $Y$ by $h(S,X)$ in the HTE estimators. Still, while new in our setting, these learners serve as natural baselines and provide useful intuition for the challenges of HLTE estimation, and thus motivate our \methods.

%$\bullet$\,\textbf{T-learner:} First, the response functions $\mu_a(x)=\E\left[h(S,x)|A=a, X=x,R=0\right], \;a\in\{0,1\}$ are estimated. Then, a simple plug-in estimator is obtained as $\hat\tau^0_{T}(x)=\hat\mu_1(x) - \hat\mu_0(x)$. We name it the long-term T-learner following the standard HTE setting~\cite{Kunzel.2019}.
...
$\bullet$\,\textbf{RA-learner:} The RA-learner follows a two-stage pseudo-outcome regression. It first $(\hat{h}, \hat\mu_1, \hat\mu_0)$ in the first stage. In the second stage, it regresses the regression-adjusted pseudo-outcome:
%\footnotesize
%\begin{align}
$    \mathcal{T}_\mathrm{RA}= A(h(S,X)-\hat\mu_0(X)) + (1-A)(\hat\mu_1(X)-h(S,X))$.
%\notag
%\end{align}
%\normalsize

$\bullet$\,\textbf{IPW-learner:} The learner uses inverse-propensity-based the pseudo-outcome:
%\begin{align}
$    \mathcal{T}_\mathrm{IPW} = \left(\frac{A}{\hat\pi(X)}-\frac{1-A}{1-\hat\pi(X)}\right)h(S,X) $.
%\end{align}
...
Furthermore, the above learners are sensitive to the nuisance estimation errors $\hat\eta-\eta$. The underlying reason is that they are not Neyman-orthogonal (i.e. $\E[D_\eta\mathcal{T}(Z;\eta)]\neq 0$), and thus any error from $\hat\eta$ will have a first-order impact on the pseudo-outcome~\cite{Chernozhukov.2018}. 
...
%\end{align}
Under certain assumptions, this brings favorable properties such as a higher-order convergence rate for the estimator. 
%Hence, orthogonality has a core impact on the robustness of learner in the two-stage training.
...
\begin{definition}[Long-term DR-pseudo-outcome]
    \emph{Let $Z=(X,A,S,R,Y)$ be the full set of variables, then the DR-pseudo-outcome in the surrogacy model is 
    \begin{equation*}
    \label{DR-pseudo-outcome}
    \mathcal{T}_\mathrm{DR}(Z;\eta)=\1(R=0) \hat{\tau}_\mathrm{AIPW}(Z;\eta) + \1(R=1)\psi_\mathrm{obs}(Z;\eta) ,
\end{equation*}
where $\hat{\tau}_\mathrm{AIPW}(Z;\eta)$ and $\psi_\mathrm{obs}(Z;\eta)$ are defined as
\begin{align*}
    \hat{\tau}_\mathrm{AIPW}&(Z;\eta) = \mu(1,X) - \mu(0,X) %\label{tau-AIPW-def}
    \\ 
    &\quad\qquad +\frac{A-\pi(X)}{\pi(X)(1-\pi(X))}(h(S,X) - \mu(A,X)) , \notag \\
    \psi_\mathrm{obs} =& \frac{1-\rho(S,X)}{\rho(S,X)} \left( \frac{\pi_s(S,X) - \pi(X)}{\pi(X)(1-\pi(X))} \right)
...
%    \begin{equation}
        $\mathcal{L}_\mathrm{DR}(g;\eta)=\E_n\left[(g(X)-\mathcal{T}_\mathrm{DR})^2\mid R=0\right], \; g \in \mathcal{G} $.
%    \end{equation}
...
\emph{Problems of the DR-learner:} However, the DR-learner uses inverse propensities to construct the pseudo-outcomes, which leads to large finite-sample variance~\cite{Fisher.2023}. In Appendix~\ref{app:variance}, we analyze the variance of the long-term DR-pseudo-outcome and explained quantitatively why it has high variance when there is low overlap. As a result, the long-term DR-learner is not robust to low overlaps.



%In the following section, we address the low overlap issue by developing a weighted, Neyman-orthogonal learner.

\section{Our LT-learners}
\label{sec:method}

\textbf{Main idea of our LT-learners:} The key idea is to downweight the sample with (i)~low treatment or (ii)~low long-term outcome overlap. %This reweighting strategy is widely used in HTE estimation to stabilize the learner~\cite{Fisher.2023, Frauen.2025-model, Frauen.2025-survival}. 
...
\textbf{Overview:} In Section~\ref{subsec:ortho-weight-loss}, we show how we orthogonalize the oracle weighted loss. In Section~\ref{subsec:estimating-hlte}, we give the algorithm for training the HLTE estimator. Then, in Section~\ref{subsec:instantiation}, we give specific instantiations of our LT-learners for different types of low-overlap regimens. Finally, in Section~\ref{subsec:convergence}, we show that the estimator achieves quasi-oracle convergence rate under mild conditions.
...
\end{equation}
Here, for simplicity, we denote $\omega(x)=\omega(\pi(x),\rho(x))$. The basic ideas behind are: (1)~If $\mathcal{G}$ is a class of constant function, then $g^*(x) \equiv \tau^0_\omega$ is the minimizer of $\mathcal{L}_\omega^{\text{oracle}}(g)$. (2)~The Gateaux derivative of the EIF w.r.t. the nuisance functions $\eta$ are zero~\cite{Chernozhukov.2018}. Thus, EIF-based pseudo-outcomes often satisfy Neyman-orthogonality and have conditional mean $\E[Y(1)-Y(0)\mid X,R=0]$, such as the DR-pseudo-outcome~\cite{Kennedy.2020-DR-learner}.\footnote{See Appendix~\ref{app:theory} for a theoretical background.} See Appendix~\ref{app:eif} for the EIF.


\begin{contributionbox}
\textbf{LT-learners:} Based on the derived EIF, we propose our orthogonal loss as follows:
\begin{equation}
\label{eq:weighted-ortho-loss-tau0}
\mathcal{L}_\omega(g, \eta) = \mathbb{E}_n \left[ \omega^*(Z;\eta) \left( \mathcal{T}_\mathrm{LT}(Z;\eta) - g(X) \right)^2 \right],
\end{equation}
...
\begin{align}
\vspace{-0.3cm}
    \mathcal{T}_\mathrm{LT}&(Z;\eta) = \frac{1}{\omega^*(Z;\eta)} \Bigg[ \1(R=0) \omega(X) \hat{\tau}_\mathrm{AIPW}(Z;\eta)\\
     +  R& \cdot\omega(X) \psi_\mathrm{obs}(Z;\eta) + (\mu(1,X)-\mu(0,X)) \Omega(Z;\eta) \Bigg].\notag
\end{align}
\end{contributionbox}
\normalsize
Intuitively, $\omega^*$ consists of a main part $\omega(x)$ and a residual-based $\Omega(Z;\eta)$\footnote{$\Omega(Z;\eta)$ can be viewed as a mean $0$ term that corrects the first-order bias of the weight estimator $\hat\omega(X)=\omega(\hat\pi(X),\hat\rho(X))$.} and thus $\omega^*\approx \omega$ (from average). Take $\omega=\pi(1-\pi)\rho$ as an example: When either 1) treatment overlap is low (small $\pi(1-\pi)$) or 2) long-term outcome overlap is low (small $\rho$), the sample is downweighted in the pseudo-outcome regression in Eq.~\ref{eq:weighted-ortho-loss-tau0} via $\omega^*\approx \omega$. 
...
\begin{align}
    \E&\left[\omega^*(Z;\eta)\mid X\right] = \omega(X) , \notag \\
    \E&\left[\mathcal{T}_\mathrm{LT}(Z;\eta) \mid X,R=0\right] = \E\left[ \phi_\mathrm{DR}\mid X,R=0\right]=\tau^0(X) . \notag
\end{align}
\end{remark}
...
\begin{theorem}[Neyman-orthogonality]
    ...
    The proposed loss $\mathcal{L}_\omega(g, \eta)$ is orthogonal w.r.t. to the nuisance $\eta$.
    \vspace{-0.1cm}
\end{theorem}
\begin{proof}
...
\begin{theorem}[Oracle equivalence]
    ...
    \begin{equation}
        \argmin_{g\in \mathcal{G}} \mathcal{L}_\omega(g, \eta) = \argmin_{g \in \mathcal{G}} \mathcal{L}_\omega^{\mathrm{oracle}}(g, \eta).
    \end{equation}
...
Specifically, we randomly split the samples from $\mathcal{D}=\{Z_i\}_{i=1}^{N}$ into two disjoint parts $\mathcal{D}=\mathcal{D}_A \cup \mathcal{D}_B$, with $|\mathcal{D}_A|=N-n, \mathcal{D}_B=n$. Then, our LT-learners proceed as follows

\textbf{Stage \circlednumblue{1}:} We fit the nuisance functions on $\mathcal{D}_A$ data with arbitrary machine learning models to achieve the nuisance estimators $\hat\eta=(\hat\pi, \hat\pi_s,\hat\rho,\hat\rho_s, \hat{h}, \hat\mu)$. %In stage 2, we calculate the estimated weighting function
...
\label{eq:empirical-loss}
    \hat{g}=\argmin_{g\in\mathcal{G}}\mathcal{L}_{\omega, n}(g, \hat\eta) = \sum_{i=1}^n \hat\omega^*_i (g(X)- \hat{\mathcal{T}}_i)^2 .
\end{align}
...
$\bullet$\,\textbf{TO-LT-learner:} This variant is designed for when only treatment overlap (TO) is problematic (e.g., because $\mathcal{D}_1$ is small or high-dimensional, so that some subgroups have a low propensity score). We choose the weighting function $\omega(x)=\pi(x)(1-\pi(x))$, which downweights samples with limited treatment overlap. We provide the formulation of TO-LT-learner in 

Under this choice, the corresponding long-term weighting function simplifies to $\omega^*=\1(R=0)(A-\pi(X))^2$, and therefore assigns zero weight to observations with $R=1$. In this case, we multiply $\omega^*$ into the squared term in Eq.~(\ref{eq:weighted-ortho-loss-tau0} to avoid division by zero. As a result, the second-stage objective reduces to a special case that is equivalent to an R-learner-style objective~\cite{Nie.2021} but adapted to the long-term setting:

\vspace{-0.5cm}
\footnotesize
\begin{align}
\label{t-learner-loss}
 & \mathcal{L}_{\pi(1-\pi)}(g;\eta)=\mathbb{E}\Bigl[ \1(R=0)((h(S,X)-m(X))- \\
& -(A -\pi(X))g(X)\big))^2 - 2\1(R=1)\psi_{t}(Z;\eta)\,g(X)\Bigr] \notag ,
\end{align}
\vspace{-0.5cm}
\normalsize
...
$\bullet$\,\textbf{LO-LT-learner: } This learner is intended for settings where only long-term outcome overlap (LO) is low. In this case, we choose the weighting function $\omega(x)=\rho(x)$ to downweight samples with a low probability of observing long-term outcomes. Then, the corresponding LR-weighting function and the LT-pseudo-outcome can be computed via
...
&\omega^* = (R-\rho(X))^2,\\
    &\mathcal{T}_\mathrm{LT}=\mu(1,X)-\mu(0,X) \notag \\
    &+ \begin{cases} 
      \frac{1}{\rho(X)}\left( \hat{\tau}_\mathrm{AIPW} - (\mu(1,X)-\mu(0,X)) \right), & \text{if } R=0, \\
      \frac{\rho(X)}{(1-\rho(X))^2} \psi_\mathrm{obs}(Z;\eta) , & \text{if } R=1 .
   \end{cases}
\end{align*}
\normalsize
$\bullet$\,\textbf{DO-LT-learner:} Finally, we present an instantiation of our estimator for settings in which both treatment overlap and long-term outcome overlap is low. Here, we choose the weighting function $\omega(x)=\pi(x)(1-\pi(x))\rho(x)$, which jointly downweights samples with weak treatment overlap and low probability of observing long-term outcomes. The resulting LT weights and LT pseudo-outcomes are given by
...
%\label{hlte-learner-mixed}
   ...
   \end{cases} \\
    ...
    &+ \begin{cases} 
      \frac{\rho(A-\pi)}{\omega^*(Z;\eta)} \big( h(S,X) - \mu(A,X) \big) , & \text{if } R=0 , \\
      \frac{\rho}{(1-\rho)^2} \psi_\mathrm{obs}(Z;\eta) , & \text{if } R=1 .
   \end{cases}
\end{align*}
\vspace{-0.5cm}
\normalsize
...
$    \|\hat{g}-g_0\|^2 \lesssim R_g + \|\hat\mu-\mu\|_4^2(\|\hat\rho-\rho\|_4^2 + \|\hat\pi-\pi\|_4^2) \notag %\\
    %&
    + \|\hat{h}-h\|_4^2( \|\hat\pi_s-\pi_s\|_4^2 + \|\hat\rho_s-\rho_s\|_4^2)$,
%\end{align}
    where $R_g=\mathcal{L}(\hat{g}, \hat{\eta}, \lambda\{\hat{\pi}(X)\}) - \mathcal{L}(g_0, \hat{\eta}, \lambda\{\hat{\pi}(X)\})$ is the quasi-oracle rate.
\end{theorem}
\vspace{-0.5cm}
\begin{proof}
...
%    \begin{align}
$        \max \{ k_\mu+k_\pi, k_\mu+k_\rho, s_h+s_\pi,s_h+s_\rho \} \leq r_g$.
%    \end{align}
    \vspace{-0.2cm}
\end{corollary}
...
\begin{table*}[t] 
    ...
    \begin{minipage}[t]{0.48\textwidth}
        ...
        \resizebox{\textwidth}{!}{%
            \begin{tabular}{@{}l l l l l@{}}
                ...
                T-learner           & $0.32 \pm 0.04$ & $0.95 \pm 0.14$ & $1.32 \pm 0.21$ & $1.85 \pm 0.42$ \\
                RA-learner          & $0.31 \pm 0.04$ & $0.92 \pm 0.16$ & $1.28 \pm 0.31$ & $1.74 \pm 0.49$ \\
                IPW-learner         & $0.36 \pm 0.08$ & $2.05 \pm 1.15$ & $1.63 \pm 1.05$ & $4.40 \pm 1.70$ \\
                DR-learner          & $\mathbf{0.25 \pm 0.06}$ & $1.38 \pm 0.75$ & $1.85 \pm 0.82$ & $3.32 \pm 1.87$ \\
                \midrule
                TO-LT-learner (\textit{ours})   & $0.27 \pm 0.03$ & $\mathbf{0.63 \pm 0.18}$ & $1.12 \pm 0.38$ & $1.25 \pm 0.25$ \\
                LO-LT-learner (\textit{ours})   & $0.26 \pm 0.04$ & $0.94 \pm 0.12$ & $0.83 \pm 0.22$ & $1.28 \pm 0.32$ \\
                DO-LT-learner (\textit{ours}) & $0.28 \pm 0.03$ & $0.64 \pm 0.09$ & $\mathbf{0.77 \pm 0.17}$ & $\mathbf{0.87 \pm 0.21}$ \\ \midrule
    ...
    \vspace{0pt}
        ...
        \begin{tabular}{@{}l l l l l@{}}
            ...
            T-learner & $0.68 \pm 0.11$ & $1.88 \pm 0.13$ & $2.55 \pm 0.36$ & $3.75 \pm 0.58$ \\
            RA-learner& $0.65 \pm 0.13$ & $1.74 \pm 0.23$ & $2.26 \pm 0.46$ & $2.95 \pm 0.63$ \\
            IPW-learner & $0.74 \pm 0.33$ & $4.15 \pm 2.06$ & $3.35 \pm 1.93$ & $9.20 \pm 3.23$ \\
            DR-learner & $0.52 \pm 0.23$ & $2.75 \pm 1.43$ & $3.62 \pm 1.49$ & $6.75\pm 3.48$ \\
            \midrule
            TO-LT-learner (\textit{ours}) & $0.55 \pm 0.10$ & $\mathbf{0.85 \pm 0.14}$ & $2.25 \pm 0.53$ & $2.45 \pm 0.62$ \\
            LO-LT-learner (\textit{ours}) & $\mathbf{0.51 \pm 0.11}$ & $1.92 \pm 0.45$ & $1.65 \pm 0.16$ & $2.62 \pm 0.59$ \\
            DO-LT-learner  (\textit{ours}) & $0.57 \pm 0.09$ & $0.89 \pm 0.15$ & $\mathbf{1.58 \pm 0.21}$ & $\mathbf{1.75 \pm 0.16}$ \\ \midrule
...
There is no direct benchmark specifically designed for our HLTE setting. Therefore, as baselines, we derive and adapt standard meta-learners, namely, a T-learner, RA-learner, IPW-learner, and DR-learner to our setting (see Section~\ref{sec:standard_learners}; details in Appendix~\ref{app:baselines}). %All baselines are model-agnostic (e.g., arbitrary neural network architectures can be used) and differ only in their adjustment strategy, which allows us to isolate the effect of our orthogonal loss with overlap weighting. 
...
\underline{\emph{Results:}} The results are in Table~\ref{tab:synthetic-main}. We find: \textbf{(1)}~As suggested by our theory, our \methods outperforms all other meta-learners on $\mathcal{D}_t, \mathcal{D}_o, \mathcal{D}_{t+o}$, i.e., whenever at least one type of overlap issue is present. In contrast, the classical T-, RA-, IPW-, and DR- learners perform worse in these low-overlap settings. \textbf{(2)}~In line with our design, the DO-LT-learner performs particularly well in the mixed overlap setting ($\mathcal{D}_{t+o}$), the OO-LT-learner in the low-treatment overlap ($\mathcal{D_{o}}$), etc. \textbf{(3)}~Overall, our \methods based on overlap weighting consistently outperform baselines, which highlights the benefits of Neyman-orthogonality and sample weighting in low-overlap settings.  
...
\begin{wrapfigure}{r}{0.3\textwidth} 
  ...
  Low overlap leads to a high finite-sample variance of the DR-learner, while our \methods is robust and maintains a low variance.}
  \label{fig:estimator-variance-overlap}
  \vspace{-0.4cm}
\end{wrapfigure}

%\textbf{$\bullet$\,Overall performance underf overlap:} We simulate synthetic data to mimic four types of overlap scenarios: a) no overlap issue, b) treatment overlap, c) observation overlap, and d) mixed overlap. Under each scenarios, we evaluate the performance of three instantiations of our \methods. 
...
\textbf{$\bullet$\,Ablations wrt. orthogonality: } In this experiment, we analyze why Neyman-orthogonality is crucial for robustness. Here, we vary the sample sizes, where a lower sample size implies that the nuisance function estimation is more difficult. In addition to the existing baselines, we added two ablations: weighted but non-orthogonal versions of the RA-learner and DR-learner (see Appendix~\ref{app:baselines} for details). These ablations also employ overlap-based retargeting, allowing us thus to isolate the effect of Neyman-orthogonality. \underline{\textit{Results:}} The left panel of Figure~\ref{fig:neyman} shows that the orthogonal DO-LT-learner consistently outperforms non-orthogonal ablations in finite-sample regimes. The right panel further illustrates how nuisance function estimation performance deteriorates as the sample size decreases. Taken together, these results show that our orthogonal \methods remains robust, thus effectively mitigating the bias induced by nuisance estimation errors.
...
\begin{figure}
    ...
    \caption{\textbf{Benefit of Neyman-orthogonality:} (\textit{Left})~PEHE vs. sample size. The orthogonal \methods outperforms the weighted (but non-orthogonal) DR/T-learner. (\textit{Right})~Means squared error (MSE) of the nuisance estimation. Taken together, high errors in the nuisance functions at small sample sizes drive the instability of non-orthogonal baselines.}
    \label{fig:neyman}
    \vspace{-0.5cm}
\end{figure}

\textbf{Experiments with medical data:} We evaluate the performance of the meta-learners under different types of overlaps in a medical context. Specifically, we consider a surrogate variable (7-day outcome) and a long-term outcome (6-month recovery status) based on the real-world patient covariates from the Third International Stroke Trial (IST-3)~\cite{Sandercock.2008-IST}. We again create 4 scenarios: $\mathcal{D}_*, \mathcal{D}_t, \mathcal{D}_o, \mathcal{D}_{t+o}$, which correspond to no overlap issue, low treatment overlap only, low long-term outcome overlap only, and mixed overlap issue (see Appendix~\ref{app:semi-synthetic} for details).  \underline{\textit{Results:}} Table~\ref{tab:semi-syn-main} shows the mean and standard deviation of PEHE on semi-synthetic dataset. While all models perform similarly under ideal overlap ($\mathcal{D}_*$: no overlap issue), our proposed learners are superior in low-overlap regimens. Notably, in the scenario with both overlap issues ($\mathcal{D}_{t+o}$), our DO-LT-learner reduces the estimation error by $40.6\%$ compared to the best baseline (T-learner). This again demonstrates that our proposed learners are highly effective in the presence of overlap issues.
...
We do not forsee any negative impact from our work.
...
Specifically, let $\mathcal{T}_\mathrm{DR}(Z)$ be the uncentered EIF for the average effect, we use it to approximate the unobserved individual treatment effect $Y(1)-Y(0)$ because:
\begin{align}
    \mathbb{E}[\mathcal{T}_\mathrm{DR}(Z) \mid X=x] = \tau^0(x).
\end{align}
By defining a loss function $\mathcal{L}(g) = \mathbb{E}[(g(X) - \phi(Z))^2]$, the minimizer is the conditional expectation $\mathbb{E}[\phi(Z)|X]$, which recovers $\tau^0(x)$. Crucially, because $\mathcal{T}_\mathrm{DR}(Z)$ inherits the orthogonality property of the EIF, the second stage regression retains robustness against nuisance estimation errors. 

\paragraph{Semantics of Neyman-orthogonality: Functional vs. Loss} In this paper we frequently uses the concept of Neyman-orthogonality. We point out that actually they differ in their target object and semantic implication:
...
\begin{align}
    ...
    \psi_{obs}(Z;\eta) &= \frac{1-\rho(S,X)}{\rho(S,X)} \left( \frac{\pi(S,X) - \pi(X)}{\pi(X)(1-\pi(X))} \right) \big( Y - h(S,X) \big) \\
...
\label{lemma:orthogonality}
    $\phi(Z; \eta)$ is he efficient influence function, which satisfies the Neyman-orthogonality condition with respect to the complete set of nuisance functions $\eta = (\pi(x), \pi(s,x), \rho(x), \rho(s,x), h(s,x), \mu(a,x))$. That is, the G\^ateaux derivative of the expected EIF vanishes at the true nuisance parameters:
    \begin{align}
...
In the main text, we provide three instantiations, which are the TO-LT-learner, the LO-LT-learner and the DO-LT-learner. In fact, there are more instatiations that are also found to be helpful:

\textbf{LO$\frac{1}{2}$-LT-learner:} We consider an alternative weighting function $\omega(x)=\sqrt{\rho(x)}$ to provide a smaller penalty for low long-term outcome overlap. The corresponding LT-weighting function and LT-pseudo-outcome are given by:
...
\textbf{(2)  $\E\left[\1(R=1)\psi_{obs}(Z;\eta) \mid X\right] = 0$}:
\begin{align}
\label{eq:psi-mean-zero-property}
    \E\left[\1(R=1)\psi_{obs}(Z;\eta) \mid X\right] &= \rho(X) \E\left[\frac{1-\rho(S,X)}{\rho(S,X)} \left( \frac{\pi(S,X) - \pi(X)}{\pi(X)(1-\pi(X))} \right) \big( Y - h(S,X) \mid X, R=1\right] \notag \\
    &= \rho(X) \E_S\left[ \frac{1-\rho(S,X)}{\rho(S,X)} \left( \frac{\pi(S,X) - \pi(X)}{\pi(X)(1-\pi(X))} \right)\E_Y\left[ \big( Y - h(S,X) \mid S, X, R=1\right] \mid X, R=1\right]=0.
\end{align}

\textbf{(3) $\E\left[\hat{\tau}_{AIPW}(Z;\eta)\mid X, R=0\right]=\tau^0(X)$}:
\begin{align}
...
\footnotesize
\begin{align*}
    \var(\mathcal{T}_\mathrm{DR} \mid X) \leq \underbrace{(1-\rho(X)) \cdot \Sigma_{t}(X)}_{\text{treatment overlap}} + \underbrace{\rho(X) \cdot \Sigma_{\text{o}}(X)}_{\text{long-term outcome overlap}}
\end{align*}
\vspace{-0.3cm}
\normalsize
\vspace{-0.6cm}
...
\begin{align*}
    ...
    \Sigma_{\text{o}}(X) &= \mathbb{E}_{S|X} \left[ \left(\frac{1-\rho(S,X)}{\rho(S,X)}\left(\frac{\pi_s-\pi}{\pi(1-\pi)}\right)\right)^2 \sigma_y(S,X) \right],
\end{align*}
where $\sigma_h(X,A)=\var(h(S,X)|X,A,R=0)$, $\sigma_y(S,X)=\var(Y|S,X,R=1)$.
\normalsize
\end{lemma}

This is similar to the variance analysis from \citet{Fisher.2023}; see Appendix~\ref{app:variance} for a formal proof. When the treatment overlap $\pi(x)(1-\pi(x)$ is close to zero, both $\Sigma_t(x)$ and $\Sigma_o(x)$ become very large. When there is limited long-term outcome overlap, namely $\rho(x)$ close to zero (and thus for $\rho(s,x)$ as well),  we have that $\rho(x)\Sigma_o(x)$ scales roughly with $\E[\frac{1}{\rho(S,x)}|X=x]$. Therefore, $\mathcal{T}_\mathrm{DR}$ has a high variance when at least one of the overlaps is low. As a result, the orthogonal learner (i.e., DR-learner) for long-term outcomes is \emph{not} robust against low overlap.
...
\textbf{Part (B): Observation Overlap Instability ($R=1$).}
In the observational dataset ($R=1$), $\mathcal{T}_{\mathrm{DR}}$ uses the observation proxy $\psi_{\mathrm{obs}}$. Let:
$$
Z_1 = \frac{1-\rho(S,X)}{\rho(S,X)} W_{\pi}(S,X) (Y - h(S,X)),
$$
where $W_{\pi}(S,X)=\left( \frac{\pi(S,X) - \pi(X)}{\pi(X)(1-\pi(X))} \right)$.
...
\begin{align}
    \Sigma_o(X) := \var(Z_1 \mid X) = \mathbb{E}_{S|X} \left[ \left( \frac{1-\rho(S,X)}{\rho(S,X)} \right)^2 \left(\frac{\pi(S,X) - \pi(X)}{\pi(X)(1-\pi(X))} \right)^2 \var(Y \mid S, X) \right]
\end{align}
This term scales with $\frac{1}{\rho(S,X)^2}$, causing high variance under limited observation overlap.
...
By definition, the loss in Eq.\ref{eq:weighted-ortho-loss-tau0} is orthogonal if we can show that its functional derivative w.r.t. $g$ is Neyman-orthogonal:
\begin{align}
...
\end{align}
Using Bayes' rule $\frac{dP(S|A=1)}{dP(S|R=0)} = \frac{\pi(S,X)}{\pi(X)}$, we have $\mu_{\delta_h}(1) - \mu_{\delta_h}(0) = \mathbb{E}_{S|X}\left[ \delta_h(S,X) \frac{\pi(S,X) - \pi(X)}{\pi(X)(1-\pi(X))} \right]$.
For the $\psi_{obs}$ term,  we make use of the density ratio property that $\frac{dP(S \mid R=1, X)}{dP(S \mid R=0, X)} = \frac{\rho(S,X)}{1-\rho(S,X)} \frac{P(R=0|X)}{P(R=1|X)}$ to transform the measure from the observational to the experimental sample:
\begin{align}
    D_2 &= -\mathbb{E}\left[ \1(R=1)\omega(X) \frac{1-\rho(S,X)}{\rho(S,X)} \frac{\pi_s(S,X)-\pi(X)}{\pi(1-\pi)} (-\delta_h) \right] \notag \\
    &= \mathbb{E}_{X}\left[\E_S\left[ \rho(X) \frac{1-\rho(S,X)}{\rho(S,X)} \omega(X) \frac{\pi_s(S,X) - \pi}{\pi(1-\pi)} \delta_h \mid X, R=1 \right]\right] \notag \\
...
F_4:=-\E\Big[\big((\mu(1)-\mu(0))-g\big)\,\partial_r\Omega_r\Big|_{r=0}\Big]
&=
(1-\rho(X))\E\Big[\big(\tau^0(X)-g\big)\frac{\partial\omega}{\partial\pi}(X)\delta_\pi(X)\Big].
\end{align}

Putting everything together, we have
\begin{align}
    \partial_r \E[S(g,\eta+r\delta_\pi)]\Big|_{r=0}=&F_1 + F_2+F_3+F_4 \notag \\
    =& \E\Big[\1(R=0)\frac{\partial\omega}{\partial\pi}(X)\,\delta_\pi(X) (g(X)-\tau^0(X))\Big] + \E\Big[(1-\rho(X))\frac{\partial\omega}{\partial\pi}(X)\,\delta_\pi(X)\big(\tau^0(X)-g\big)\Big] \notag \\
    =&0.
\end{align}

\textbf{4. Orthogonality w.r.t. $\pi_s(s,x), \rho(s,x)$:}

Let $\pi_r(S,X)=\pi_s(S,X)+r\,\delta_{\pi_s}(S,X)$. Since $\pi_s(S,X)$ only appears in $\psi_{obs}$, therefore
\begin{align}
\partial_r\psi_{obs,r}\Big|_{r=0}
=\left(\partial_r (\frac{1-\rho(S,X)}{\rho(S,X)} \left( \frac{\pi_s(S,X) - \pi(X)}{\pi(X)(1-\pi(X))} \right))\Big|_{r=0}\right)\big(Y-h(S,X)\big).
\end{align}
...
\end{align}
The same derivation applies for $\rho(s,x)$.
...
\begin{align}
    \E\left[\1(R=1)\psi_{obs}(Z;\eta) \mid X\right] &= \E\left[ \1(R=1) \frac{1-\rho(S,X)}{\rho(S,X)} \left( \frac{\pi(S,X) - \pi(X)}{\pi(X)(1-\pi(X))} \right) \big( Y - h(S,X) \big) \Bigg| X \right] \\
    &= \E\left[ \1(R=1) \frac{1-\rho(S,X)}{\rho(S,X)} \left( \frac{\pi(S,X) - \pi(X)}{\pi(X)(1-\pi(X))} \right) \underbrace{\E[Y - h(S,X) \mid S, X, R=1]}_{=0} \Bigg| X \right] \\
    &= 0.
\end{align}
...
\begin{align}
\label{taylor-exp}
     \mathcal{L}(\hat{g}, \hat{\eta}, \lambda\{\hat{\pi}(X)\}) = \mathcal{L}(g_0, \hat{\eta}, \lambda\{\hat{\pi}(X)\}) + D_g\mathcal{L}(g_0, \hat{\eta}, \lambda\{\hat{\pi}(X)\})[\hat{g} - g_0] + \frac{1}{2} D_g^2 \mathcal{L}(\bar{g}, \hat{\eta}, \lambda\{\hat{\pi}(X)\})[\hat{g} - g_0, \hat{g} - g_0].
\end{align}
...
\begin{equation}
    \frac{\alpha}{2} \|\hat{g}-g_0\|_2^2 \leq \underbrace{\mathcal{L}(\hat{g}, \hat{\eta}, \lambda\{\hat{\pi}(X)\}) - \mathcal{L}(g_0, \hat{\eta}, \lambda\{\hat{\pi}(X)\})}_{:=R_g \text{ oracle excess risk}} - D_g\mathcal{L}(g_0, \hat{\eta}, \lambda\{\hat{\pi}(X)\})[\hat{g} - g_0]
\end{equation}
In the following proof, we show that $D_g\mathcal{L}(g_0, \hat{\eta}, \lambda\{\hat{\pi}(X)\})[\hat{g} - g_0]$ can be decomposed into second-order cross multiplication of nuisance errors.
\begin{align}
    D_g\mathcal{L}(g_0, \hat{\eta}, \lambda\{\hat{\pi}(X)\})[\hat{g} - g_0]=& \E\left[ \underbrace{\1(R=0) \hat{\omega}(X) \hat{\tau}_{AIPW} + \1(R=1) \hat{\omega}(X) \hat\psi_{obs}}_{\text{Term (1)}} + \underbrace{\big( (\hat\mu_1-\hat\mu_0\big) \hat{\Omega} - \hat{\omega^*}g_0}_{\text{Term (2)}}\right]
\end{align}
...
\begin{align}
    ...
    =& \tau^0(X) + \text{Bias}(\mu, \pi) + \underbrace{\E\left[ \left( \frac{\pi_0(S,X)}{\hat{\pi}(X)} - \frac{1-\pi_0(S,X)}{1-\hat{\pi}(X)} \right) \delta_h(S,X) \mid X, R=0 \right]}_{\text{First-order bias}}.
\end{align}
...
\label{eq:tauDR-bound}
    ...
    + \E\left[ \1(R=0)\left( \frac{\pi_0(S,X)}{\hat{\pi}(X)} - \frac{1-\pi_0(S,X)}{1-\hat{\pi}(X)} \right) \delta_h(S,X) \mid X\right]
\end{align}

Then, we analyze the correction term $\hat\psi_{obs}$. We let $\gamma(s,x):=\frac{1- \rho(s,x)}{\rho(s,x)}$ and analyze the conditional expectation of $\hat{\omega}(X) \hat\psi_{obs}$ given $X$:
\begin{align}
    \E\left[\1(R=1) \hat{\omega}(X) \hat\psi_{obs} \mid X\right] =& \E\left[ \1(R=1) \hat{\omega}(X) \hat{\gamma}(S,X) \left( \frac{\hat{\pi}(S,X) - \hat{\pi}(X)}{\hat{\pi}(X)(1-\hat{\pi}(X))} \right) (Y - \hat{h}) \mid X\right] \notag \\
...
\begin{align}
    ...
    =& \E\left[ \1(R=0) \hat{\omega}(X) \left( \tau^0(X) + \text{Bias}(\mu, \pi) + \delta_h(S,X) \underbrace{ \left( \left[ \frac{\pi_0(S,X) - \hat{\pi}(X)}{\hat{\pi}(1-\hat{\pi})} \right] - \frac{\hat{\gamma}}{\gamma_0} \hat{C}_\pi \right) }_{\text{Residual } \mathcal{R}_h} \right) \right] \notag \\
    \leq & \E\left[ \1(R=0) \hat{\omega}(X) \tau^0(X) \right] + \E\left[C_0|\hat{\pi}-\pi|\left(|\hat\mu_1-\mu_1|+|\hat\mu_1-\mu_1| \right)\right] \notag \\
    & + C_h\cdot\E\left[|\hat{h}(S,X) - h(S,X)| (|\hat\pi(X) - \pi(X)| + |\hat\gamma(S,X) - \gamma(S,X)|)\right] \\
    &+ \E\left[ \1(R=0) \hat{\omega}(X) \tau^0(X) \right] + \E\left[C_0|\hat{\pi}-\pi|\left(|\hat\mu_1-\mu_1|+|\hat\mu_1-\mu_1| \right)\right] \notag \\
    &+C_h\cdot\E\left[|\hat{h}(S,X) - h(S,X)| (|\hat\pi(X) - \pi(X)| + |\hat\gamma(S,X) - \gamma(S,X)|)\right]
    \label{term-1-bound}.
\end{align}
In the last step, we define a positive constant $C_h > 0$ s.t.
\begin{align}
    \mathcal{R}_h &= \left( \left[ \frac{\pi(S,X) - \hat{\pi}(X)}{\hat{\pi}(1-\hat{\pi})} \right] - \frac{\hat{\gamma}}{\gamma_0} \hat{C}_\pi \right) \\
    & \leq \left( \left[ \frac{\pi(S,X) - \hat{\pi}(X)}{\hat{\pi}(X)(1-\hat{\pi}(X))} \right] - \frac{1-\hat{\rho}(S,X)}{1 - \rho(S,X)}  \frac{\rho(S,X)}{\hat\rho(S,X)} \frac{\hat{\pi}(S,X) - \hat{\pi}(X)}{\hat{\pi}(X)(1-\hat{\pi}(X))} \right) \notag \\
    & \leq C_h \bigl(|\hat\pi(S,X) - \pi(S,X)| + |\hat\rho(S,X) - \rho(S,X)|\bigr)
\end{align}

Now we only need to deal with the remained terms $\big( (\hat\mu_1-\hat\mu_0\big) \hat{\Omega} - \hat{\omega^*}g_0$ plus the term $\E\left[ \1(R=0) \hat{\omega}(X) \tau^0(X) \right]$. We reorganize the these terms as:
\begin{align}
...
\end{align}
Hence $\omega^*-\hat\omega$ can is of second order of the nuisance errors (via Taylor expansion around $\eta_0$):
\begin{align} 
...
\begin{align}
   ...
   &+ C_h\cdot\E\left[|\hat{h}(S,X) - h(S,X)| (|\hat\pi(X) - \pi(X)| + |\hat\rho(S,X) - \rho(S,X)|) (\hat{g}(X) - g_0(X))\right]
\end{align}
...
& + \E\left[\text{sup}_{X}|C_{\omega^*}(X) \left(\|\hat{\pi}-\pi\|^2 + \|\hat{\rho}-\rho\|^2\right)(\hat{g}(X)-g_0(X))\right] + 0.
\end{align}
Applying Cauchy-Schwarz inequality to the decomposition of $D_g\mathcal{L}_\omega(g_0, \hat{\eta})[\hat{g}-g_0]$, we have that: $\exists C>0$ s.t.
\begin{align}
    & D_g\mathcal{L}_\omega(g_0, \hat{\eta})[\hat{g}-g_0] \\
    \leq& C\cdot\Biggl(\sqrt{\E\left[(\hat\pi(X) - \pi(X))^2 (\hat\mu_1(X) - \mu_1(X))^2\right]}  +\sqrt{\E\left[(\hat\pi(X) - \pi(X))^2 (\hat\mu_0(X) - \mu_0(X))^2\right]} \notag \\
    & + \sqrt{\E\left[(\hat\rho(X) - \rho(X))^2 (\hat\mu_1(X) - \mu_1(X))^2\right]}  +\sqrt{\E\left[(\hat\rho(X) - \rho(X))^2 (\hat\mu_0(X) - \mu_0(X))^2\right]} +\notag \\
     + &\sqrt{\E\left[(\hat\pi(X) - \pi(X))^2 (\hat{h}(S,X) - h(S,X))^2\right]}  +\sqrt{\E\left[(\hat\rho(S,X) - \rho(S,X))^2 (\hat{h}(S,X) - h(S,X))^2\right]} 
    \Biggr)\|\hat{g}-g_0\|.
\end{align}
Using the AM-GM inequality on the right hand, we have that: $\forall \delta > 0$ 
\begin{align}
    & D_g\mathcal{L}_\omega(g_0, \hat{\eta})[\hat{g}-g_0] \\
    &\leq \Biggl(\frac{1}{\delta}\E\left[(\hat\pi(X) - \pi(X))^2 (\hat\mu_1(X) - \mu_1(X))^2\right] + \frac{1}{\delta} \E\left[(\hat\pi(X) - \pi(X))^2 (\hat\mu_0(X) - \mu_0(X))^2\right] \notag \\
    ...
    &+ \frac{1}{\delta} \E\left[(\hat\pi(X) - \pi(X))^2 (\hat{h}(S,X) - h(S,X))^2\right] + \frac{1}{\delta} \E\left[(\hat\rho(S,X) - \rho(S,X))^2 (\hat{h}(S,X) - h(S,X))^2\right] + 6\delta\|\hat{g}-g_0\|^2\Biggr) \cdot C
\end{align}
Substituting the inequality back to the initial equation, we have that
\begin{align}
...
\begin{align}
    \|\hat{g}-g_0\|^2 &\leq \frac{1}{\alpha/2 - 6C\delta} \Biggl(R_g + \frac{1}{\delta}\E\left[(\hat\pi(X) - \pi(X))^2 (\hat\mu_1(X) - \mu_1(X))^2\right] + \frac{1}{\delta} \E\left[(\hat\pi(X) - \pi(X))^2 (\hat\mu_0(X) - \mu_0(X))^2\right] \notag \\
    &+ \frac{1}{\delta} \E\left[(\hat\rho(X) - \rho(X))^2 (\hat\mu_1(X) - \mu_1(X))^2\right] + \frac{1}{\delta} \E\left[(\hat\rho(X) - \rho(X))^2 (\hat\mu_0(X) - \mu_0(X))^2\right] \notag \\
    &+ \frac{1}{\delta} \E\left[(\hat\pi(X) - \pi(X))^2 (\hat{h}(S,X) - h(S,X))^2\right] + \frac{1}{\delta} \E\left[(\hat\rho(S,X) - \rho(S,X))^2 (\hat{h}(S,X) - h(S,X))^2\right]\Biggr).
\end{align}
Let $\|\hat\mu-\mu\|_4=\text{max}(\|\hat\mu_1-\mu_0\|_4, \|\hat\mu_1-\mu_0\|_4)$ and use Cauchy-Schwarz inequality again, we have:
\begin{align}
    \|\hat{g}-g_0\|^2 &\lesssim R_g + \|\hat\mu-\mu\|_4^2(\|\hat\rho-\rho\|_4^2 + \|\hat\pi-\pi\|_4^2) + \|\hat{h}-h\|_4^2( \|\hat\pi-\pi\|_4^2 + \|\hat\rho_s-\rho_s\|_4^2)
\end{align}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
...
\label{eq:true-cate-simulation}
    \tau(X)=\E \left[Y^1-Y^0\mid X\right] = \E\left[b(X,S^1) - b(X, S^0) \mid X \right].
\end{equation}
...
Let $s := (S-5)/2$ be the rescaled surrogate. Define
\begin{align}
b(X,S)
&:= \gamma_0
+ \gamma_1 \exp(0.1\,s)
...
\textbf{Weighted RA-learner:} For the weighted version of the learner, we simply add weights to all the samples during the second-stage pseudo-outcome regression. Specifically, the \textit{weighted} RA-learner minimizes the following loss function:
    ...
    \end{equation}
\textbf{Weighted RA-learner:} The weighted DR-learner minimizes the following loss function in the second stage:
\begin{equation}
        \mathcal{L}_\mathrm{\omega,RA}(g;\eta)=\E_n\left[\omega(X)(g(X)-\mathcal{T}_\mathrm{DR})^2\mid R=0\right], \; g \in \mathcal{G}
\end{equation}
