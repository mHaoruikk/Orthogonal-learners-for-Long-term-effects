...
Let $X \sim \mathrm{Unif}([0,1]^{10})$, and define the treatment propensity on the experiment dataset to be:
\begin{equation}
    \pi^*(X)
    = \mathrm{trim}_{0.1}\!\big(\sin(\pi X_1 X_2)\big),
    \qquad
    \mathrm{trim}_{\eta}(u)
    := \max\{\eta,\min(u,1-\eta)\}.
\end{equation}
...
\begin{equation}
    ...
    = \sin(\pi X_1 X_2)
      ...
      + X_4 + 0.5 X_5 + X_6.
\end{equation}
We set the short-term treatment effect equal to the original simple CATE:$\tau_S^*(X) = \frac{X_1 + X_2}{2}.$ Thus,
\begin{equation}
    S
    = a^*(X)
      + (A - 0.5)\,\tau_S^*(X)
