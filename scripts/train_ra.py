...
def train_ra_learner(cfg: DictConfig, data: TwoSampleDataSplit) -> RA_Learner:
    """Train the RA_Learner along with its nuisance models."""
    logger.info("Initializing RA_Learner")
    learner = RA_Learner(cfg.model)
    learner.fit(data)
    return learner
...
def main(args: DictConfig):
    ...
    logger.info("Training RA-learner and associated nuisances")
    learner = train_ra_learner(args, train_data)
...
if __name__ == "__main__":
    ...
    # commands to run
    # python -m scripts.train_ra dataset=setupA model=RA-learner trainer=default
